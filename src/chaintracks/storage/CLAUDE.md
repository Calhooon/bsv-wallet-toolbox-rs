...
| `find_live_header_for_merkle_root(merkle_root)` | Find active header by merkle root |
| `get_headers_bytes(height, count)` | Get serialized 80-byte headers |
| `get_live_headers()` | Get all stored live headers |
| `get_available_height_ranges()` | Returns empty (no bulk storage) |
| `find_live_height_range()` | Get min/max height range of live headers |
...
`MemoryStorage` uses four `RwLock`-protected HashMaps for concurrent access:
...
3. **Previous header linking**: Finds `previous_header_id` from `previous_hash`
4. **Tip determination**: Header becomes tip if height > current tip height
5. **Reorg detection**: If new tip doesn't extend old tip, calculates reorg depth
6. **Index updates**: Updates `hash_to_id` and `height_to_id` maps
...
The module includes a basic test demonstrating insertion and retrieval:
...
async fn test_memory_storage_basic() {
    ...
    // Insert genesis, verify it becomes tip
